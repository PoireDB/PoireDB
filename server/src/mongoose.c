...
static void mg_sendnsreq(struct mg_connection *c, struct mg_str *name, int ms,
  ...
  if (dnsc->c == NULL) {
    ...
    LOG(LL_VERBOSE_DEBUG,
        ("%lu resolving %.*s @ %s, txnid %hu", c->id, (int)name->len, name->ptr,
         mg_ntoa(&dnsc->c->peer, buf, sizeof(buf)), d->txnid));
...
static size_t ff_write(void *fp, const void *buf, size_t len) {
  ...
  while ((size_t)sum < len &&
         f_write((FIL *)fp, (char *)buf + sum, sum + bs > len ? len - sum : bs,
                 &n) == FR_OK &&
         n > 0) {
    sum += n;
  }
    ...
    while ((ch = fgetc(fp)) != EOF) {
      if (intag && ch == '>' && buf[len - 1] == '-' && buf[len - 2] == '-') {
        ...
        if (sscanf(buf, "<!--#include file=\"%[^\"]", arg)) {
          char tmp[MG_PATH_MAX + BUFSIZ + 10], *p = (char *)path + strlen(path),
                                               *data;
...
int mg_lower(const char *s) { return tolower(*(const unsigned char *)s); }
